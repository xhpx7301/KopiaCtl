...
# Bootstrap KopiaCtl from the public project repository.
set -Eeuo pipefail

readonly SOURCE_URL="${KOPIACTL_SOURCE_URL:-https://raw.githubusercontent.com/xhpx7301/KopiaCtl/main/kopiactl.sh}"
readonly TEMP_DIR="$(mktemp -d)"
readonly MANAGER_SCRIPT="${TEMP_DIR}/kopiactl.sh"
...
if [[ ! -r /dev/tty ]]; then
  printf '未检测到可交互终端，无法打开 KopiaCtl 菜单。请在 SSH 终端中运行此命令。\n' >&2
  exit 1
fi

# curl | bash 会占用标准输入，菜单需要从终端读取输入。
